...
class WBK_Service_Schedule
{
    ...
    public function getDayStatus( $day )
    {
        ...
        if ( is_array( $whole_day_checkin ) ) {
            if ( in_array( $this->service_id, $whole_day_checkin ) ) {
    ...
    public function renderDayFrontend( $time_after, $offset )
    {
        ...
        for ( $i = 0 ;  $i < count( $this->timeslots ) ;  $i++ ) {
            ...
            if ( $night_houts_addon != 0 ) {
                if ( $timeslot->getStart() < $this->day + $night_houts_addon && $timeslot->getEnd() <= $this->day + $night_houts_addon ) {
    ...
    public function fableCount( $time )
    {
        ...
        $booked = $wpdb->get_var( $wpdb->prepare( "SELECT sum(quantity) FROM wbk_appointments WHERE service_id = %d AND  time = %d", $this->service_id, $time ) );
        if ( $booked === NULL ) {
            $booked = 0;
        }
        $booked2 = $wpdb->get_var( $wpdb->prepare(
            "SELECT sum(quantity) FROM wbk_appointments WHERE service_id = %d AND  ( time < %d AND ( time + %d ) > %d)",
            $this->service_id,
            $time,
            $total_duration,
            $time
    ...
    public function getAvailableCount( $time, $ignore_parital = false )
    {
        ...
        if ( get_option( 'wbk_availability_calculation_method', 'sum' ) == 'highest' ) {
            $service = new WBK_Service_deprecated();
            if ( !$service->setId( $this->service_id ) ) {
                return 0;
            }
            ...
            $total_duration = $service->getDuration() * 60 + $service->getInterval() * 60;
            $booked = $wpdb->get_var( $wpdb->prepare( "SELECT max(quantity) FROM wbk_appointments WHERE service_id = %d AND  time = %d", $this->service_id, $time ) );
            if ( $booked === NULL ) {
                $booked = 0;
            }
            $booked2 = $wpdb->get_var( $wpdb->prepare(
                "SELECT max(quantity) FROM wbk_appointments WHERE service_id = %d AND  ( time < %d AND ( time + %d ) > %d)",
                $this->service_id,
                $time,
                $total_duration,
                $time
            ...
            $avail_array = [ $booked, $booked2, $booked3 ];
            return $service->getQuantity( $time ) - max( $avail_array );
        }
        ...
        if ( get_option( 'wbk_mode_overlapping_availabiliy', 'true' ) == 'true' ) {
            $booked = $wpdb->get_var( $wpdb->prepare(
                "SELECT sum(quantity) FROM wbk_appointments WHERE ( service_id = %d AND  time = %d ) ||\r\n\t\t\t\t( service_id = %d AND  ( time < %d AND ( time + %d ) > %d)  ) ||\r\n\t\t\t\t( service_id = %d AND  ( time > %d AND time < ( %d + %d ) ) )",
                $this->service_id,
                $time,
                $this->service_id,
                $time,
        ...
        if ( $service->getQuantity( $time ) > 1 && get_option( 'wbk_gg_2way_group', 'lock' ) == 'reduce' ) {
