...
class WBK_Db_Utils
{
    ...
    static function getServices()
    {
        ...
        if ( $order_type == 'a-z' ) {
            $service_sql = "SELECT id FROM wbk_services ";
    ...
    {
        global  $wpdb ;
        if ( !WBK_Validator::validateId( $service_id, 'wbk_services' ) ) {
            return false;
        }
        ...
        foreach ( $arrIds as $service_id_this ) {
            ...
            foreach ( $timeslots_to_lock as $time_slot_start ) {
                ...
                if ( $intersect == true ) {
                    ...
                    if ( $wpdb->insert( 'wbk_locked_time_slots', array(
    ...
    static function deleteAppointmentByEmailTokenPair( $email, $token )
    {
        ...
        $count = $wpdb->get_var( $wpdb->prepare( "SELECT count(*) as cnt FROM wbk_appointments WHERE email = %s and token = %s", $email, $token ) );
        ...
        $deleted_count = $wpdb->delete( 'wbk_appointments', array(
            'email' => $email,
            'token' => $token,
        ), array( '%s', '%s' ) );
    ...
    static function deleteExpiredAppointments()
    {
        ...
        if ( get_option( 'wbk_appointments_delete_not_paid_mode', 'disabled' ) != 'disabled' ) {
            ...
            if ( $delete_rule == 'skip' ) {
                $ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM wbk_appointments where ( payment_id = '' or payment_id IS NULL ) and  ( status='pending' or status='approved'  ) and ( ( payment_method <> 'Pay on arrival' and payment_method <> 'Bank transfer' ) or payment_method IS NULL ) and  expiration_time <> 0 and expiration_time < %d", $time ) );
            } elseif ( $delete_rule == 'delete' ) {
                $ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM wbk_appointments where ( status='pending' or status='approved'  ) and ( ( payment_method <> 'Pay on arrival' and payment_method <> 'Bank transfer' ) or payment_method IS NULL ) and expiration_time <> 0 and expiration_time < %d", $time ) );
            }
            ...
            if ( $delete_rule == 'skip' ) {
                if ( count( $valid_ids ) > 0 ) {
                    foreach ( $valid_ids as $app_id ) {
                        ...
                        $wpdb->query( $wpdb->prepare( "DELETE FROM wbk_appointments where ( payment_id = '' or payment_id IS NULL ) and  ( ( payment_method <> 'Pay on arrival' and payment_method <> 'Bank transfer' ) or payment_method IS NULL ) and ( status='pending' or status='approved'  ) and  expiration_time <> 0 and expiration_time < %d and id=" . $app_id, $time ) );
                ...
                if ( count( $valid_ids ) > 0 ) {
                    foreach ( $valid_ids as $app_id ) {
                        ...
                        self::deleteAppointmentDataAtGGCelendar( $app_id );
                        self::copyAppointmentToCancelled( $app_id, __( 'Auto', 'wbk' ) );
                        $wpdb->query( $wpdb->prepare( "DELETE FROM wbk_appointments where  ( status='pending' or status='approved'  ) and ( ( payment_method <> 'Pay on arrival' and payment_method <> 'Bank transfer' ) or payment_method IS NULL ) and expiration_time <> 0 and expiration_time < %d and id=" . $app_id, $time ) );
                        $noifications->sendOnCancelCustomer();
                        $noifications->sendOnCancel();
                        switch_to_locale( $current_locale );
                        date_default_timezone_set( 'UTC' );
        ...
        if ( WBK_Validator::checkInteger( $pending_expiration, 5, 500000 ) ) {
            ...
            $ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM wbk_appointments where ( status='pending' ) and created_on  < %d", $old_point ) );
            $valid_ids = array();
            foreach ( $ids as $appointment_id ) {
                if ( WBK_Validator::validateId( $appointment_id, 'wbk_appointments' ) ) {
                    $valid_ids[] = $appointment_id;
            ...
            if ( count( $valid_ids ) > 0 ) {
                foreach ( $valid_ids as $app_id ) {
                    ...
                    $noifications->prepareOnCancelCustomer();
                    $noifications->prepareOnCancel();
                    $wpdb->query( $wpdb->prepare( "DELETE FROM wbk_appointments where  ( status='pending' )   and created_on  < %d and id  = " . $app_id, $old_point ) );
                    $noifications->sendOnCancelCustomer();
                    $noifications->sendOnCancel();
                    switch_to_locale( $current_locale );
                    date_default_timezone_set( 'UTC' );
        ...
        if ( get_option( 'wbk_gdrp', 'disabled' ) == 'enabled' ) {
            $sql = "DELETE FROM wbk_appointments WHERE time < %d";
            $sql = apply_filters( 'wbk_gdpr_query', $sql );
            $wpdb->query( $wpdb->prepare( $sql, $time ) );
            $sql = "DELETE FROM wbk_cancelled_appointments WHERE time < %d";
            $sql = apply_filters( 'wbk_gdpr_query', $sql );
            $wpdb->query( $wpdb->prepare( $sql, $time ) );
        }
    ...
    )
    {
        ...
        return self::message_placeholder_processing(
            $message,
            $appointment,
            $service,
            $total_amount,
    ...
    )
    {
        ...
        $one_slot_price = str_replace( '#price', number_format(
            $service->getPrice( $appointment->getTime() ),
            get_option( 'wbk_price_fractional', '2' ),
            get_option( 'wbk_price_separator', '.' ),
            ''
        ), $price_format );
    ...
    )
    {
        ...
        if ( get_option( 'wbk_multi_booking', 'disabled' ) != 'disabled' ) {
            ...
            $thanks_message = self::message_placeholder_processing(
                $thanks_message,
                $appointment,
                $service,
                $total_price,
    ...
    public static function getAppointmentMomentPrice( $appointment_id )
    {
        ...
        $sql = $wpdb->prepare( "SELECT moment_price FROM wbk_appointments WHERE id = %d", $appointment_id );
    ...
    public static function getIPByAppointmentId( $appointment_id )
    {
        ...
        $sql = $wpdb->prepare( "SELECT user_ip FROM wbk_appointments WHERE id = %d", $appointment_id );
        $status = $wpdb->get_var( $sql );
        return $status;
    }
    ...
    static function doCacheForGoogleCalendars()
    {
        ...
        foreach ( $calendar_ids as $calendar_id ) {
            if ( !WBK_Validator::validateId( $calendar_id, 'wbk_gg_calendars' ) ) {
                continue;
            }
            $google = new WBK_Google();
    ...
    static function getCountOfAppointmentsByEmailTimeService( $email, $time, $service_id )
    {
        ...
        $count = $wpdb->get_var( $wpdb->prepare(
            " SELECT COUNT(*) FROM wbk_appointments WHERE email=%s and time=%d and service_id=%d",
            $email,
            $time,
            $service_id
        ) );
    ...
    static function getCountOfAppointmentsByEmailService( $email, $service_id )
    {
        global  $wpdb ;
        $count = $wpdb->get_var( $wpdb->prepare(
            " SELECT COUNT(*) FROM wbk_appointments WHERE email=%s and service_id=%d and time > %d",
            $email,
            $service_id,
            time()
        ) );
    ...
    static function getCountOfAppointmentsByEmailServiceDay( $email, $service_id, $day )
    {
        global  $wpdb ;
        $count = $wpdb->get_var( $wpdb->prepare(
            " SELECT COUNT(*) FROM wbk_appointments WHERE email=%s and service_id=%d and day = %d",
            $email,
            $service_id,
            $day
        ) );
    ...
    {
        global  $wpdb ;
        $count = $wpdb->get_var( $wpdb->prepare( " SELECT COUNT(*) FROM wbk_appointments WHERE  day = %d", $day ) );
        return $count;
    }
    ...
    {
        global  $wpdb ;
        $lang = $wpdb->get_var( $wpdb->prepare( 'select service_category from wbk_appointments where id = %d', $app_id ) );
        return $lang;
    }
    ...
    static function getPymentItemNamesByAppoiuntmentIds( $apppointment_ids )
    {
        ...
        foreach ( $apppointment_ids as $appointment_id ) {
            ...
            $item_name = self::message_placeholder_processing( $item_name, $appointment, $service );
    ...
    static function copyAppointmentToCancelled( $appointment_id, $cancelled_by )
    {
        ...
        $appointment_data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM wbk_appointments  WHERE id = %d", $appointment_id ), ARRAY_A );
        $wpdb->insert( 'wbk_cancelled_appointments', array(
            ...
            'payment_cancel_token' => $appointment_data['payment_cancel_token'],
            'admin_token'          => $appointment_data['admin_token'],
            'expiration_time'      => $appointment_data['expiration_time'],
            'time_offset'          => $appointment_data['time_offset'],
            'gg_event_id'          => $appointment_data['gg_event_id'],
            'coupon'               => $appointment_data['coupon'],
        ...
        do_action( 'wbk_table_after_add', [ $wpdb->insert_id, 'wbk_cancelled_appointments' ] );
    }
    ...
    static function getDurationOfAppointment( $appointment_id, $cancelled = FALSE )
    {
        ...
        if ( !$cancelled ) {
            ...
            $value = $wpdb->get_var( $wpdb->prepare( " SELECT duration FROM wbk_appointments WHERE id = %d ", $appointment_id ) );
        } else {
            if ( !WBK_Validator::validateId( $apppointment_id, 'wbk_cancelled_appointments' ) ) {
                return 0;
            }
            $value = $wpdb->get_var( $wpdb->prepare( " SELECT duration FROM wbk_cancelled_appointments WHERE id = %d ", $appointment_id ) );
        }
    ...
    static function getCouponName( $coupon_id )
    {
        ...
        $name = $wpdb->get_var( $wpdb->prepare( " SELECT  name from wbk_coupons WHERE id = %d", $coupon_id ) );
        return $name;
    }
    ...
    {
        $format = WBK_Date_Time_Utils::getDateFormat();
