...
class WBK_Email_Notifications {
	...
	public function send_gg_calendar_issue_alert_to_admin(){
		...
		$headers = 'From: ' . $this->from_name . ' <' . $this->from_email .'>' . "\r\n";
		add_filter( 'wp_mail_content_type', array( $this, 'set_email_content_type' ) );
    	wp_mail( $service->getEmail(), 'Issue with the Google calenar intgration.', 'Webba Booking plugin was unable to connect with the Google Calendar, please check your settings.', $headers );
		remove_filter( 'wp_mail_content_type', array( $this, 'set_email_content_type' ) );
	}
}
