...
class WBK_Backend_Schedule extends WBK_Backend_Component
{
    ...
    public function enqueueScripts()
    {
        ...
        if ( isset( $_GET['page'] ) && $_GET['page'] == 'wbk-schedule' ) {
            ...
            wp_enqueue_script(
                'wbk-schedule',
                plugins_url( 'js/wbk-schedule.js', dirname( __FILE__ ) ),
    ...
    public function ajaxScheduleLoad()
    {
        ...
        for ( $i = 1 ;  $i <= 7 ;  $i++ ) {
            ...
            $html .= '<div class="day_container">' . '<div id="day_title_' . $day_to_render . '" class="day_title ' . $statusClass . '">' . wp_date( $date_format, $day_to_render, new DateTimeZone( date_default_timezone_get() ) ) . '</div>' . $html_day_controls . '
								<div>' . $html_schedule . '</div>
						</div>';
    ...
    public function ajaxLockDay()
    {
        ...
        if ( $wpdb->insert( 'wbk_days_on_off', array(
    ...
    public function ajaxPrepareAppointment()
    {
        ...
        $resarray = array(
            ...
            'timestamp'    => $time,
            'quantity'     => $quantity,
            'available'    => $appointment_available,
    ...
    public function ajaxAddAppointment()
    {
        ...
        do_action( 'wbk_table_after_add', array( $id, 'wbk_appointments' ) );
        ...
        Plugion()->set_value(
            ...
            'appointment_status',
            $id,
            get_option( 'wbk_appointments_default_status', 'approved' )
        );
        Plugion()->set_value(
            'wbk_appointments',
    ...
    public function ajaxCreateMultipleBookingsAutoProcessing()
    {
        ...
        foreach ( $times as $time ) {
            ...
            do_action( 'wbk_table_after_add', array( $appointment_id, 'wbk_appointments' ) );
            $auto_lock = get_option( 'wbk_appointments_auto_lock', 'disabled' );
            if ( $auto_lock == 'enabled' ) {
