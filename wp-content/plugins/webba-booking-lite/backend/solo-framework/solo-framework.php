...
add_filter( 'wp_default_editor', 'slf_default_edior'  );
add_filter( 'tiny_mce_before_init', 'slf_customizeEditor'   );


function slf_default_edior(){
	return 'tinymce';
}
...
function slfLoadAssets(){
	if ( isset( $_GET[ 'page' ] ) && ( $_GET[ 'page' ] == 'wbk-appearance'   ||
		$_GET[ 'page' ] == 'wbk-appointments-old' ||
		$_GET[ 'page' ] == 'wbk-email-templates' ||
		$_GET[ 'page' ] == 'wbk-service-categories'  ||
		$_GET[ 'page' ] == 'wbk-gg-calendars'||
		$_GET[ 'page' ] == 'wbk-coupons-old'   ) ) {

        wp_enqueue_style( 'wbk-backend-style', plugins_url( '../css/wbk-backend.css', __FILE__ ), array(), '4.0.18' );
	    wp_enqueue_style( 'slf-default-style', plugins_url( '/css/slf-default.css', __FILE__ ), array(), '3.3.85' );
	...
	if ( isset( $_GET[ 'page' ] ) &&   $_GET[ 'page' ] == 'wbk-appointments-old' ) {
	...
	if ( isset( $_GET[ 'page' ] ) &&   $_GET[ 'page' ] == 'wbk-coupons-old' ) {
...
class SoloFramework extends stdClass  {
	...
	public function renderSectionSetControls( $slug ){
		...
		$presets_folder =  WP_WEBBA_BOOKING__PLUGIN_DIR . DIRECTORY_SEPARATOR . 'presets' . DIRECTORY_SEPARATOR.'*';
	...
	public function update_3_3_71(){
		if( isset(  $this->sections_sets[ 'wbk_extended_appearance_options' ]->sections['section_timeslots']->components['timeslot_disabled_button_background'] ) ){
			return;
		}
		// disabled time slot background
		$component = new SLFColor( array( 'name' => __( 'Disabled button background', 'wbk' ),
	   												    'desc' => __( 'Disabled time slot button background', 'wbk' ),
		  											    'slug' => 'timeslot_disabled_button_background',
													    'value' => '#b4b4b4',
	 												    'css_class' => 'wbk-slot-disabled-button',
	 												    'css_prop' => 'background-color'
	 												   ));
		$this->sections_sets[ 'wbk_extended_appearance_options' ]->sections['section_timeslots']->addCompontnet( $component );
		$component = new SLFColor( array( 'name' => __( 'Disabled button color', 'wbk' ),
														  'desc' => __( 'Disabled time slot button color', 'wbk' ),
														  'slug' => 'timeslot_disabled_button_color',
														  'value' => '#ffffff',
		 												  'css_class' => 'wbk-slot-disabled-button',
		 												  'css_prop' => 'color'
		 												   ));
