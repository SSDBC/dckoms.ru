...
{
    if ( $table_name == 'wbk_appointments' || $table_name == 'wbk_cancelled_appointments' ) {
...
{
    if ( $slug != 'wbk_appointments' && $slug != 'wbk_cancelled_appointments' ) {
        return;
    }
    ?>
        <script>
...
function wbk_plugion_on_after_row_add( $table_name, $table_name_not_filtered, $row )
{
    ...
    if ( $table_name == 'wbk_appointments' ) {
...
function wbk_plugion_on_before_row_delete( $table_name, $table_name_not_filtered, $row )
{
    ...
    if ( $table_name == 'wbk_services' ) {
        $wpdb->query( $wpdb->prepare( 'DELETE from wbk_appointments where service_id = %d', $row->id ) );
    }
}
...
function wbk_plugion_field_can_add( $input, $field_name, $table_name )
{
    ...
    if ( $table_name == 'wbk_cancelled_appointments' ) {
        return [];
    }
    return $input;
}
...
function wbk_plugion_row_can_delete( $input, $row, $table_name )
{
    ...
    if ( $table_name == 'wbk_appointments' ) {
        ...
        if ( in_array( 'administrator', $user->roles, true ) || is_multisite() && super_admin() ) {
            return true;
        } else {
