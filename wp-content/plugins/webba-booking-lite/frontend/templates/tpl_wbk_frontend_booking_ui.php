				...
				if( $found_valid_appointments == 0 ){
					...
					if( $title != '' ){
						if( get_option( 'wbk_allow_coupons', 'disabled' ) == 'enabled' ){
							$title = '<input class="wbk-input" id="wbk-coupon" placeholder="' . __( 'coupon code', 'wbk' ) . '" >' . $title;
						}
					}
			...
			<div class="wbk-col-12-12" >
				 <?php
				 	if ( $data[0] <> 0 ){
				 		...
				 		echo '<input type="hidden" data-attribute_name="service-id" id="wbk-service-id" data-multi-low-limit="' . $service->getMultipleLowLimit() . '" data-multi-limit="' . $service->getMultipleLimit() . '" value="' . $data[0] . '" />';
